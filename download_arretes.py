...
def parse_plain_list(driver, elt):
    ...
    for e_it in elt.find_elements_by_xpath('./li'):
        e_text = e_it.get_attribute('textContent')
        ...
        rlimits = ['Arrêté', 'Arrrété', 'arreté', 'Arrété', 'Arrête',
                   'Main Levée', 'Main levée', 'Main-Levée', 'main levée', 'Mainlevée',
                   'Modification', 'Abrogation', 'abrogé',
                   'Interdiction']
...
def parse_accordion_list(driver, elt):
    ...
    for e_acc in elt.find_elements_by_xpath('./div'):
        ...
        docs.extend([(e_key, x[0], x[1], x[2], x[3]) for x in e_docs])
    return docs
...
def parse_arretes(driver, url, outdir):
    ...
    for section in cont_div.find_elements_by_xpath('./h4'):
        ...
        # puis du contenu, directement ou dans une liste d'accordéons (un par
        # arrondissement)
        if next_elt.tag_name == 'ul':
            # * soit <ul>
            docs = parse_plain_list(driver, next_elt)
            # on ne connaît pas l'arrondissement
            res.extend([(doc_class, '', x[0], x[1], x[2])
                        for x in docs])
        elif next_elt.tag_name == 'p':
            # * soit <p><div[contains(@id, "dexp-accordions-wrapper")]>"
...
def dump_doc_list(docs, fn_out):
    ...
    colnames = ['classe', 'arrondissement', 'adresse', 'item',
                'nom_doc', 'url']
...
if __name__ == "__main__":
    ...
    fn_out = 'mrs-arretes-de-peril.csv'
    dump_doc_list(docs, fn_out)
    # on télécharge vraiment les documents
    # TODO
